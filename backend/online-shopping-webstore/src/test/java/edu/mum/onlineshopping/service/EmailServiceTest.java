...
//			order.setId(i+1);
	...
	public void testSendSimpleMessage() {
		TinyEmailObject emailObject = TinyEmailObject.buildEmailObject("ducvo1983@gmail.com", "Test", "Test sending simple message");
		assertTrue(emailService.sendSimpleMessage(emailObject));
		emailObject.setTo("bademail@badmail.com");
		assertTrue(emailService.sendSimpleMessage(emailObject));
		assertFalse(emailService.sendSimpleMessage(null));
	}

	@Test
	public void testSendOrderMessageUsingTemplate() {
		for(Order order : testOrder) {
			assertTrue(emailService.sendOrderMessageUsingTemplate(order));
		}
		assertFalse(emailService.sendOrderMessageUsingTemplate(null));
	}

	@Test
	public void testSendUserRegisterMessageUsingTemplate() {
		for(Person p : testPerson) {
			assertTrue(emailService.sendUserRegisterMessageUsingTemplate(p));
		}
		assertFalse(emailService.sendUserRegisterMessageUsingTemplate(null));
	}
	...
	public void testSendMessageWithAttachment() {
		...
		assertFalse(emailService.sendMessageWithAttachment(null,null,null));
	}
